...
the member name. */
select distinct f.name, m.firstname, m.surname from Members as m
...
SELECT m.surname, m.firstname, m.recommendedby AS recomender_id, r.surname as rec_surname, r.firstname as rec_firstname
FROM Members AS m
...
FROM (
SELECT month(b.starttime) AS months, memid
FROM Bookings b
WHERE memid !=0
) AS b
GROUP BY b.months;
